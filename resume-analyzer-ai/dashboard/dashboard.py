...
def get_top_rated_resumes(self, limit=5):
    """Get top rated resumes based on ATS score"""
    cursor = self.conn.cursor()
    try:
        cursor.execute('''
        SELECT 
        ...
        FROM resume_data rd
        LEFT JOIN resume_analysis ra ON rd.id = ra.resume_id
        ORDER BY ra.ats_score DESC
        LIMIT ?
        ''', (limit,))
        
        return cursor.fetchall()
    except Exception as e:
        print(f"Error fetching top rated resumes: {str(e)}")
        return []
...
def render_top_rated_section(self):
    """Render top rated resumes section"""
    st.markdown("<h2 class='section-title'>🏆 Top Rated Resumes</h2>", unsafe_allow_html=True)
    
    # Get top rated resumes
    top_rated = self.get_top_rated_resumes()
    
    if top_rated:
        # Convert to DataFrame
        columns = ['Name', 'Target Role', 'Target Category', 'ATS Score', 'Keyword Match', 'Format Score', 'Section Score']
        df = pd.DataFrame(top_rated, columns=columns)
        ...
        with st.container():
            st.markdown('<div class="top-rated-data">', unsafe_allow_html=True)
            
            # Display the top-rated resumes
            st.dataframe(df, use_container_width=True)
            
            st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.info("No top-rated resumes available")
...
def render_dashboard(self):
    ...
    # Render the existing sections
    self.render_dashboard_header()
    self.render_quick_stats()
    self.render_performance_analytics()
    self.render_key_insights()
    
    # Render the new Top Rated section
    self.render_top_rated_section()
    
    # Render admin section if applicable
    if st.session_state.get('is_admin', False):
        self.render_admin_section()
